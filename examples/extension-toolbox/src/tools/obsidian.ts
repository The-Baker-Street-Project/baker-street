...
function resolve(vaultPath: string, notePath: string): string {
...
export function registerObsidianTools(server: McpServer): void {
  ...
  if (!vaultPath) {
    console.warn('[toolbox] OBSIDIAN_VAULT_PATH not set — Obsidian tools disabled');
    return;
  }
  ...
  server.tool(
    ...
    async ({ path }: { path: string }) => {
      try {
        const full = resolve(vaultPath, ensureMd(path));
  ...
  server.tool(
    ...
    async ({ from, to }: { from: string; to: string }) => {
      try {
        const src = resolve(vaultPath, ensureMd(from));
        const dst = resolve(vaultPath, ensureMd(to));
  ...
  server.tool(
    ...
    async ({ folder }: { folder?: string }) => {
      try {
        const dir = folder ? resolve(vaultPath, folder) : vaultPath;
  ...
  console.log('[toolbox] Obsidian tools registered (10 tools)');
}
