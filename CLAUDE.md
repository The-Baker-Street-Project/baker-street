...
This tags the release, pushes to GitHub, and triggers the CI/CD pipeline which builds all images to GHCR, runs acceptance tests, and publishes installer binaries to the GitHub Release. The release manifest (`manifest.json`) is generated automatically by CI.
...
All secrets live in `.env-secrets` (gitignored). `scripts/deploy-all.sh` auto-sources this file.
...
AUTH_TOKEN                # API auth token (auto-generated by deploy-all.sh if not set)
...
`AUTH_TOKEN` is a 32-byte hex token auto-generated by `scripts/deploy-all.sh` on first run and persisted to `.env-secrets`. It protects the Brain API:
